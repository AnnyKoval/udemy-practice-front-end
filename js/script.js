document.addEventListener("DOMContentLoaded", function (event) {
  ...
  endProposal.innerHTML = `${deadline.replace(/\-/g, ":")} o 00:00`; //замена дефиса на двоеточие
  ...
  function getDynamicInformation(selector) {
    const input = document.querySelector(selector); //получаем инфо из инпутов

    input.addEventListener("input", () => {
      if (input.value.match(/\D/g)) {
        input.style.border = "1px solid red"; //в случае ввода не числа - красный border
      } else {
